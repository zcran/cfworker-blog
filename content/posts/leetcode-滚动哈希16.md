...
impl Solution {
    ...
    pub fn beautiful_indices(s: String, a: String, b: String, k: i32) -> Vec<i32> {
        ...
        // 如果 a 或 b 为空，按题意应返回空（题目未明确，但通常模式非空）
        ...
        // 对每个 a 的位置，检查是否存在 b 的位置距离 <= k
        pos_a
            .into_iter()
            .filter(|&i| {
                // 在 pos_b 中二分查找第一个 >= i 的位置
                match pos_b.binary_search(&i) {
                    Ok(_) => true, // 相等，距离 0 <= k
                    Err(idx) => {
                        // idx 是第一个大于 i 的位置
                        let left_ok = idx > 0 && i - pos_b[idx - 1] <= k;
                        let right_ok = idx < pos_b.len() && pos_b[idx] - i <= k;
                        left_ok || right_ok
                    }
                }
            })
